...
for(i in 1:length(mukey)){
	...
	tmp = unlist(list(
		...
		sand = sum(componentSummaryTable$Weight*componentSummaryTable$sand,na.rm=T),
		silt = sum(componentSummaryTable$Weight*componentSummaryTable$silt,na.rm=T),
		clay = sum(componentSummaryTable$Weight*componentSummaryTable$clay,na.rm=T),
