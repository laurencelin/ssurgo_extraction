...
mukey_texture = read.csv(soil_mukey_texture, stringsAsFactors=F)
	...
	cat_texture = as.integer(mukey_texture[cond,'soilhorizon'])
	rast$soil_texture = cat_texture[cond2]; writeRAST(rast,'soil_horizonClass',zcol='soil_texture',overwrite=T)
